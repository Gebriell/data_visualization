...
const Question4 = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/q4/${selectedDataset}`)
  ...
  useEffect(() => {
    ...
    svg
      ...
      .text((d) =>
        formatPercentage(`${(d.endAngle - d.startAngle) / (2 * Math.PI)}`)
      )
  ...
  const dataArray = Array.from(chartData).map(([property, value]) => ({
    property,
    value,
  }));
